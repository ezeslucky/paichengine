...
opts.Add(BoolVariable("production", "Set defaults to build Patch for use in production", False))
...
opts.Add("vsproj_name", "Name of the Visual Studio solution", "patch")
...
if env["platform"] in compatibility_platform_aliases:
    ...
    print_warning(
        f'Platform "{alias}" has been renamed to "{platform}" in Patch 4. Building for platform "{platform}".'
    )
...
if cc_version_major == -1:
    ...
    if cc_version_major < 9:
        print_error(
            ...
            "C++17, or has bugs when compiling Patch. Supported versions are 9 "
    ...
    if cc_version_major == 16 and cc_version_minor < 11:
        print_error(
            ...
            "when compiling Patch. Use a newer VS2019 version, or VS2022."
        )
        Exit(255)
    if cc_version_major == 15 and cc_version_minor < 9:
        print_error(
            "Detected Visual Studio 2017 version older than 15.9, which has bugs "
            "when compiling Patch. Use a newer VS2017 version, or VS2019/VS2022."
        )
        Exit(255)
    if cc_version_major < 15:
        print_error(
            "Detected Visual Studio 2015 or earlier, which is unsupported in Patch. "
